using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Microsoft.Samples.VisualStudio.IronPython.Project
{
    static class IPyConstants
    {
        public const string pythonFileExtension = ".py";
    }

    static class GuidList
    {
        public const string guidPythonProjectPkgString = "38F7EBA3-A5C6-4e32-B2C9-B6456F31B129";
        public const string guidIronPythonProjectCmdSetString = "22EBFCA0-A97A-4ad3-AC19-77BEC7E8C0EB";

        public const string guidPythonProjectFactoryString = "AF48B115-53DB-4e4f-A04C-CF2B83C29EE3";

        public const string guidIronPythonLanguageString = "ae8ce01a-b3ff-4c19-8c80-54669c197f2c";

        public const string libraryManagerGuidString = "F12CCE33-BE39-4605-9B3E-5801E026D8A8";
        public const string libraryManagerServiceGuidString = "3F8033D8-BB88-47ec-9430-03D8B55A393E";
    }
}
